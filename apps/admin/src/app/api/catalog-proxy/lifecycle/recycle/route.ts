...
/**
 * POST /api/catalog-proxy/lifecycle/recycle — any live state → RECYCLED (terminal). Admin role: irreversible.
 * Pass-through to /api/v1/admin/lifecycle/recycle (relayer tier upstream — the relayer key
...
export async function POST(req: Request) {
  ...
  const payload = { tokenId, reason: reason.trim() };
  const res = await templatesUpstream("/api/v1/admin/lifecycle/recycle", { method: "POST", body: payload, relayer: true });
