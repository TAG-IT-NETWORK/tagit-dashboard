 ...
 *
 * SCOPE — deliberately narrow. The `matcher` below covers ONLY /asset/<tokenId>
 * and /api/asset/<tokenId>: pure on-chain state lookups with no cryptographic
...
import { isCacheableReadPath, neverCacheControl } from "@/lib/cache";
import {
  READ_RATE_LIMIT,
...
export const config = {
  // Token-id reads only. See the scope note above before widening this.
  //
  ...
  matcher: ["/asset/:path*", "/api/asset/:path*"],
};
...
export async function middleware(request: NextRequest) {
  ...
  // cacheable-surface predicate so a future matcher edit can't silently start
  // caching a tap route.
  if (!isCacheableReadPath(pathname)) return NextResponse.next();
  ...
  if (!decision.allowed) {
    ...
    return new NextResponse("Too many requests. This is a public read endpoint; please slow down.", {
      status: 429,
      headers: {
        "content-type": "text/plain; charset=utf-8",
        "cache-control": neverCacheControl(),
        "retry-after": String(decision.resetSeconds),
        ...rateLimitHeaders(decision.remaining, decision.resetSeconds),
      },
    });
  }
  ...
  // below → `no-store`.
  for (const [k, v] of Object.entries(rateLimitHeaders(decision.remaining, decision.resetSeconds))) {
    response.headers.set(k, v);
  }
  return response;
}

function rateLimitHeaders(remaining: number, resetSeconds: number): Record<string, string> {
  return {
